...
## Gen 2 PITR Availability
{: #pitr-gen2-availability}

Point-in-Time Recovery (PITR) for {{site.data.keyword.databases-for-mysql_full}} Gen 2 will be **available in Q3 hardening**.

At GA, Gen 2 provides snapshot-based backups with 30-day retention. PITR functionality, which allows recovery to any point in time within a 7-day window, will be added in Q3 hardening.

Once available, PITR will offer:
- Recovery to any point in time within the last 7 days
- Continuous incremental backups
- Transaction replay capability for precise recovery points
- Granular recovery options for disaster recovery scenarios

The _Backups_ tab of your deployment's UI will display PITR information under _Point-in-Time recovery_ once the feature is available.

## PITR Overview (Available in Q3 Hardening)
{: #pitr-overview}
...
To verify the correct recovery time, check the database logs. Checking the database logs requires the [Logging integration](/docs/databases-for-mysql-gen2?topic=databases-for-mysql-gen2-logging&interface=cli) to be set up on your deployment.
